...
**Line-level highlights** – prefix at start of line, applies to end of line:
...
**Inline highlights** – use `@X{text}` to highlight a span within a line.
Works inside code blocks (indented or fenced):

```
00000000   @g{07 7E 00 00} @b{00 EC 00 00} - AA 40
```

Both forms use the same color letters (`@g`, `@b`, `@r`, etc.).

---

### `markdownfilter` 🪄

**Apache CGI Filter** – Renders `README.md` (and other `.md` files) live in
the browser when Apache is configured to pipe `.md` requests through this
script. It applies the same colour-prefix highlights and blank-line-before-list
fix-up as `b4markdown`, plus mermaid block handling and chat-transcript
prettifying.

**Manual installation required:**

`markdownfilter` is **not** installed by `make install_all`. Install it by
hand into your CGI directory (e.g. `~/.cgi-bin/`) as a relative symlink so
edits in this repository take effect immediately:

```bash
ln -s ../autoreadme/markdownfilter ~/.cgi-bin/markdownfilter
```

Then point Apache at it via something like:

```apache
Action markdown-filter /cgi-bin/markdownfilter
AddHandler markdown-filter .md
```
...
| `THEMES_CSS` | Themes CSS file path (light/dark selector) | /.themes.css |
| `SETTINGS_ICON` | Icon for settings gear (random or specific emoji) | random |
...
| markdown.css          | 🎨 | Base CSS for generated HTML (precedence over .markdown.css) |
| markdown.rf.css       | 🎨 | Right-frame CSS variant used when images are present        |
| markdownfilter        | 🔩 | Apache CGI filter for live README.md rendering              |
...
| themes.css            | 🎨 | Theme definitions (light/dark) with CSS variables           |
